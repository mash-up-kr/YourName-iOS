...
final class RemoteDataLoaderTests: XCTestCase {
  ...
  // 🧪 System Under Test
  var dataLoader: RemoteDataLoader!
  ...
  override func setUp() {
    ...
    dataLoader = RemoteDataLoader(session: mockURLSession)
  }
  ...
  func test_load의_응답의_HTTPStatusCode가_500이면_Error를_뱉어야한다() {
    ...
    let task = dataLoader.loadData(with: mockURLRequest) { result in
      expectation.fulfill()
      switch result {
      case .success:
        XCTFail()
